                    ...
                    struct n3iwf_dp_gtpu_view *view)
{
    ...
    while (next_extension != 0) {
        ...
        if (current_type == N3IWF_DP_GTPU_PSC) {
            qfi = packet[offset + 2] & 0x3fU;
            direction = (packet[offset + 1] & 0xf0U) == 0x10U ?
                        N3IWF_DP_UPLINK : N3IWF_DP_DOWNLINK;
        }
                   ...
                   struct n3iwf_dp_gre_view *view)
{
    ...
    view->qfi = (uint8_t)(view->key & 0x3fU);
    if (view->qfi == 0) {
        return -ENOMSG;
    }
